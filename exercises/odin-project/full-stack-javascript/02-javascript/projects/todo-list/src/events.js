...
eventBus.addEventListener('eventbus-event', (e) => {
  ...
  // sets todo only when target triggers todo change
  if (validTodoTargets.includes(action)) {
  ...
  if (action === 'add-project-modal-add-button') {
    ...
    form.reset();
    modal.close();
    redraw({ viewName: 'single', pIndex: -1 });
  }
  if (action === 'add-todo-modal-add-button') {
    ...
    projects.at(pIndex).addTodo(todoTitle, todoDescription, todoPriority);
    modal = document.querySelector('dialog');
    form = document.querySelector('form');
    form.reset();
    modal.close();
