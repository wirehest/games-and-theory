...
export type Location = Path | Stream | Town | Clearing;

export type Path = Basic & {
	type: 'path';
	through: Location;
	shortcut?: Town;
};

export type Town = Basic & {
	type: 'town';
	around?: Location;
	through?: Location;
};

export type Clearing = Basic & {
	type: 'clearing';
	through?: Location;
};

export type Stream = Basic &
